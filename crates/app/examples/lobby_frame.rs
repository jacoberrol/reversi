//! Render the real lobby UI offscreen to `target/lobby.png` (`just lobby-frame`),
//! so we can inspect its look without a live window. Uses `app::lobby` — the
//! same theme + layout the live app draws.
...
fn main() {
    ...
    let state = LobbyState {
        ...
        // Set to Some((1, "Bob".into())) to preview the incoming-invite modal.
        incoming: None,
        status: String::new(),
    };
    let mut actions: Vec<LobbyAction> = Vec::new();
    ...
    let warm = ctx.run(raw.clone(), |ctx| lobby::ui(ctx, &state, &mut actions));
    let output = ctx.run(raw, |ctx| lobby::ui(ctx, &state, &mut actions));
    ...
    std::fs::write("target/lobby.png", &png).expect("write");
    println!("wrote target/lobby.png ({} bytes)", png.len());
}
